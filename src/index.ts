...
api.get('/fcm/status/:studentNum', c =>
  c.get('fcmController').getTokenStatus(c)
);
api.delete('/fcm/unregister/:studentNum', c =>
  c.get('fcmController').unregisterToken(c)
);
api.post('/fcm/test-push/:studentNum', c =>
  c.get('fcmController').sendTestPush(c)
);
